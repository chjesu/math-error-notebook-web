...
| 推荐、复习计划和 PDF | `services/web_domain/` | 仅已验证且授权题可推荐；推荐、复习和 PDF 本地链路已验收，题库为空时展示缺口。按中国标准时间为每个账号记录唯一学习用量：每日判题建议 24 道、硬上限 40 道，可完整覆盖一套 21 道试卷并保留订正余量；每日推荐建议 12 道、硬上限 24 道。每日复习 PDF 一次可选 12 道错题，每条只配 1 道推荐训练题；错题原题始终展示以说明推荐依据，但只有第 1、2 阶段或上次复习部分正确/错误时标记需重做并留作答区，其余阶段仅作推荐依据。题库推荐不足时回退为重做原题。重复提交、失败和无法识别不计数，已经开始处理的同一批图片完整完成后才停止下一批。桌面 Skill `output/pdf` 中的历史 PDF 可通过既有错题迁移脚本幂等同步到个人练习 PDF 历史，重名内容只存一份但保留每个生成记录的文件名 |
