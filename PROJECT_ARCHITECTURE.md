...
| 文件上传、解析、审核、判题 | API + Harness 附件业务桥 + 现有错题领域服务 | 会话每条消息最多上传 10 张 PNG/JPEG；每张图片可按阅读顺序拆出至多 20 道题及其对应作答。官方 Harness 模型先按图片分别冻结 OCR，再一次提交本批结构化结果；Host 逐张读取真实附件，Python 服务复用文件隔离、题目冻结、判题候选和自动入本。用户明确说明改错、订正或重新判时，未完成复习组允许以新冻结候选替换旧单题判定并保留候选链审计；已完成阶段不由模型自动回滚。正确题自动跳过，错误或部分正确自动入本，只有无法识别或证据冲突才等待补充。若保守匹配到已验证、当前且授权的题库版本，则先对冻结的独立答案执行确定性比对；按小题、无序备选答案和等价区间/连写不等式规范化后仍无法确认等价时，才把题库当前版本答案与解析作为受保护证据交给同一会话做第二阶段语义复核。语义复核一致时绑定 `question_id` 并继续入本；确认实质不一致时，以题库当前答案与解析重新判定学生作答并记录覆盖证据；只有仍无法确认、版本变化或授权失效时禁止入本。附件哈希和冻结版本保证重试不重复写入，权威回执作为工具事件写回原 Harness 会话；学生发现误判时可在原会话中一次确认后按权威 `error_id` 移除错题，无需重新上传图片，账号归属、确认原文和错题编号均由确定性代码复核，同时取消相关待复习任务及未使用推荐题。每张图片判题及必要复核后，工具结果和最终回复都依据真实回执给出一个可立即执行的下一步。模型不可用时安全停止，PDF/DOCX 自动解析与生产异步 Worker 仍延期 |
...
“本地完整测试版”当前指：登录/注册→个人空工作台→上传 PNG/JPEG→立即创建可恢复批次→后台拆题、独立求解和判题→正确题自动跳过、错误或部分正确自动入本→仅已验证推荐→复习→PDF；浏览器用 SSE 恢复进度，工作者崩溃按持久租约和有限重试恢复，过期 token 不得写回。无法识别、证据冲突或模型最终失败时安全结束，不猜测、不误写。用户仍可在同一会话中修正和追问。导出/注销的敏感 OTP 二次验证、会话撤销、下载和任务/文件失效也必须通过本地 smoke 与独立安全复核。该闭环和权威题库本地同步不等于生产完成；真实短信/CAPTCHA/KMS/OSS、生产 Worker 生命周期和压测、PDF/DOCX 自动解析、运行中追加/重新生成/分叉、灾备/观测和生产恢复仍是门禁。
