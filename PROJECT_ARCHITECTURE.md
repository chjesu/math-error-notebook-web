...
| 文件上传、解析、审核、判题 | API + Worker + OSS | 上传、手工候选、确认、手工判题候选和入本已在本地完整可用；自动解析/判题 Worker 待接入 |
| 推荐、复习计划和 PDF | `services/web_domain/` | 已实现本地闭环；只推荐已验证授权题，缺题时明确返回缺口 |
| 前端/PWA、运营后台和运维恢复 | `web/`、后续运维模块 | 响应式手工学习闭环前端已实现；PWA、运营和生产恢复待完成 |
...
“本地学习闭环已跑通”不等于“整个 Web 项目完成”。当前可认定手机号直达、文件上传、手工题干/作答候选、手工判题候选、确认入本、已验证推荐、分阶段复习和默认无答案 PDF 已在本地 MySQL/HTTP 跑通。手工路径是自动解析/判题不可用时的明确降级，不伪造模型结果。实际含题目的权威库迁移、自动解析/判题 Worker、导出/注销、生产运维恢复、性能与独立安全复核仍须完成。
