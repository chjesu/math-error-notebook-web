...
| 前端/PWA、运营后台和运维恢复 | `web/`、后续运维模块 | 六个侧边栏入口保持独立 URL/HTML；工作台使用单聊天流承载附件、模型候选、人工确认和入本结果，复用同一领域状态机；PWA、运营后台、生产恢复延期 |
