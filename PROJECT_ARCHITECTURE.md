...
| 前端/PWA、运营后台和运维恢复 | `@deepseek-ai/dsh-web-frontend`、`extensions/dsh-math-notebook-ui/`、`web/`、`services/web_ops/` | 工作台已直接复用固定版 DeepSeek Harness 官方 Web 前端和 Host，会话树、历史分页、附件、输入、上下文计量及压缩不再由项目仿写；Harness 内部固定使用隔离的“错题会话”工作区，学生界面不显示工作区选择和添加入口。错题本、练习 PDF 和学习进度通过 Harness 的 `conversation` 扩展面载入内容视图；学生设置仅显示账号与隐私。独立 `/admin` 入口已实现按角色过滤的脱敏、只读运营视图，覆盖用户管理、最近 7 天业务行为、Harness 供应商实报 Token、失败任务、内容质量、短信风控、隐私工单和后台访问审计，不出现在学生导航中，也不提供业务写操作。Harness 会话与登录账号一对一绑定，Token 累计值幂等写入 MySQL；跨账号重绑失败关闭。业务服务重启后，内部工具可按持久化会话哈希恢复账号归属，不依赖易失的内存绑定。本地登录外壳在 8000 端口校验会话后嵌入隔离运行于 3080 的官方界面。PWA、人工复核提交、用户封禁/解禁、敏感访问审批、危险批量操作、生产观测和生产恢复仍延期 |
