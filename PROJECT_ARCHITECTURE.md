...
| 前端/PWA、运营后台和运维恢复 | `@deepseek-ai/dsh-web-frontend`、`extensions/dsh-math-notebook-ui/`、`web/` | 工作台已直接复用固定版 DeepSeek Harness 官方 Web 前端和 Host，会话树、历史分页、附件、输入、上下文计量及压缩不再由项目仿写；Harness 内部固定使用隔离的“错题会话”工作区，学生界面不显示工作区选择和添加入口。错题本、练习 PDF 和学习进度通过 Harness 的 `conversation` 扩展面载入内容视图；学生设置仅显示通过 `settings.section` 载入的账号与隐私，模型、插件和配置文件仍由部署端管理且不向普通用户开放。全部页面始终复用同一官方外壳，并统一采用 Harness 的字体层级、中性色、间距、圆角、按钮和状态组件规范；错题本集中今日计划、手动选题和错题档案，不再重复展示到期题目操作卡片；学习进度独立展示六阶段复习规则、记忆原理，以及按月份聚合新增错题、应复习、实际复习、需改错、逾期和阶段分布的学习日历，点击日期在原区域展开题目、错因与知识点。底层复习、进度和 PDF 接口继续复用。浏览器只访问统一的 8000 端口；该网关在服务端会话校验后代理 Harness 页面、静态资源、HTTP API 与 WebSocket，内部 Host 使用启动期随机回环端口且不向浏览器公开。PWA、运营后台和生产恢复仍延期 |
