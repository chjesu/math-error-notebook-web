...
| Web 题库、错题、作答和复习数据 | MySQL 个人 `user_id` 模型 | 本地迁移、跨用户隔离和手工闭环已验收；权威题库导入和生产回滚未完成 |
...
| 前端/PWA、运营后台和运维恢复 | `web/`、后续运维模块 | 响应式手工学习 UI 已提交；PWA、运营后台、生产恢复延期 |
...
“本地完整测试版”当前指：登录/注册→个人空工作台→人工上传→手工识别候选→确认→手工判题候选→确认入本→仅已验证推荐→复习→PDF，且导出/注销的敏感 OTP 二次验证、会话撤销、下载和任务/文件失效已通过本地 smoke 与独立安全复核。该手工闭环已验收，但不等于生产完成。真实短信/CAPTCHA/KMS/OSS/Worker、自动 OCR/AI 判题、权威题库数据导入、PWA、运营后台、压测/灾备/观测和生产恢复仍是门禁。
