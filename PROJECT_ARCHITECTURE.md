...
2026-09-03 本地 C/S 集成：浏览器只访问 8000 同源入口；Python 网关代理官方 Harness 页面、资源、API 与 WebSocket，Host 使用随机回环端口。前端依据当前 origin 访问业务页面。Qwen 的既有图片转录→判题→PDF 关联工具链保持不变；合并保留的后台批次 API 不替换默认会话流程。本次不部署 ECS，也不签署生产安全验收。第 6 节 Codex 路由为历史设计说明，不是当前工作台默认调用链。
...
| 文件上传、解析、审核、判题 | API + Harness 附件业务桥 + 现有错题领域服务 | 会话每条消息最多上传 10 张 PNG/JPEG；每张图片可按阅读顺序拆出至多 20 道题及其对应作答。官方 Harness 模型先按图片分别冻结 OCR，再一次提交本批结构化结果；Host 逐张读取真实附件，Python 服务复用文件隔离、题目冻结、判题候选和自动入本。用户明确说明改错、订正或重新判时，未完成复习组允许以新冻结候选替换旧单题判定并保留候选链审计；已完成阶段不由模型自动回滚。正确题自动跳过，错误或部分正确自动入本，只有无法识别或证据冲突才等待补充。二维码或精确题库编号命中已验证、当前且授权的题库版本时，服务端在 OCR 冻结后返回受保护的当前答案与解析，模型只做学生作答等价性核对，不再重复完整推导；无法精确命中时仍先独立解题。独立答案按小题、无序备选答案和等价区间/连写不等式规范化后仍无法与后续题库匹配确认等价时，才把题库当前版本答案与解析交给同一会话做第二阶段语义复核。语义复核一致时绑定 `question_id` 并继续入本；确认实质不一致时，以题库当前答案与解析重新判定学生作答并记录覆盖证据；只有仍无法确认、版本变化或授权失效时禁止入本。附件哈希和冻结版本保证重试不重复写入，权威回执作为工具事件写回原 Harness 会话；学生发现误判时可在原会话中一次确认后按权威 `error_id` 移除错题，无需重新上传图片，账号归属、确认原文和错题编号均由确定性代码复核，同时取消相关待复习任务及未使用推荐题。每张图片判题及必要复核后，工具结果和最终回复都依据真实回执给出一个可立即执行的下一步。模型不可用时安全停止，PDF/DOCX 自动解析与生产异步 Worker 仍延期 |
| 推荐、复习计划和 PDF | `services/web_domain/` | 仅已验证且授权题可推荐；推荐、复习和 PDF 本地链路已验收，题库为空时展示缺口。按中国标准时间为每个账号记录唯一学习用量：每日判题建议 24 道、硬上限 40 道，可完整覆盖一套 21 道试卷并保留订正余量；每日推荐建议 8 道、硬上限 24 道。每日复习 PDF 一次可选 12 道错题，每条只配 1 道推荐训练题；默认自动选题最多 8 道，轮换选择题、填空题和解答题，解答题最多 3 道，客观题不足时不以额外解答题凑满，用户仍可手动调整至 12 道。错题原题始终展示以说明推荐依据，但只有第 1、2 阶段或上次复习部分正确/错误时标记需重做并留作答区，其余阶段仅作推荐依据。题库推荐不足时回退为重做原题。同一账号在同一中国日期内，对相同选题集合和答案选项重复生成时复用已完成 PDF，不重复分配推荐题或计入推荐额度；前端在首个异步操作前锁定生成按钮。重复提交、失败和无法识别不计数，已经开始处理的同一批图片完整完成后才停止下一批。桌面 Skill `output/pdf` 中的历史 PDF 可通过既有错题迁移脚本幂等同步到个人练习 PDF 历史，重名内容只存一份但保留每个生成记录的文件名 |
| 前端/PWA 和运维恢复 | `@deepseek-ai/dsh-web-frontend`、`extensions/dsh-math-notebook-ui/`、`web/` | 工作台已直接复用固定版 DeepSeek Harness 官方 Web 前端和 Host，会话树、历史分页、附件、输入、上下文计量及压缩不再由项目仿写；Harness 内部固定使用隔离的“错题会话”工作区，学生界面不显示工作区选择和添加入口。错题本、练习 PDF 和学习进度通过 Harness 的 `conversation` 扩展面载入内容视图；学生设置仅显示账号与隐私。2026-08-31 按用户要求移除独立运营后台的页面、专用接口、查询服务和授权命令，旧 `/admin` 及 `/v1/admin/*` 返回 404。保留已执行的历史迁移及数据库记录，不再提供后台运行时。Harness 会话与登录账号一对一绑定，Token 累计值仍幂等写入 MySQL；跨账号重绑失败关闭。业务服务重启后，内部工具可按持久化会话哈希恢复账号归属，不依赖易失的内存绑定。浏览器在统一 8000 入口校验会话后由网关访问随机回环端口的官方界面；业务页面与下载也使用当前同源地址。PWA、人工复核提交、生产观测和生产恢复仍延期 |
...
## 6. Codex CLI 与质量门（历史设计，非当前工作台默认流程）
...
“本地完整测试版”当前指：登录/注册→个人空工作台→上传 PNG/JPEG→立即创建可恢复批次→后台拆题、精确参考快速核对或独立求解判题→正确题自动跳过、错误或部分正确自动入本→仅已验证推荐→复习→PDF；浏览器用 SSE 恢复进度，工作者崩溃按持久租约和有限重试恢复，过期 token 不得写回。无法识别、证据冲突或模型最终失败时安全结束，不猜测、不误写。用户仍可在同一会话中修正和追问。导出/注销的敏感 OTP 二次验证、会话撤销、下载和任务/文件失效也必须通过本地 smoke 与独立安全复核。该闭环和权威题库本地同步不等于生产完成；真实短信/CAPTCHA/KMS/OSS、生产 Worker 生命周期和压测、PDF/DOCX 自动解析、运行中追加/重新生成/分叉、灾备/观测和生产恢复仍是门禁。
...
PDF 的逐题进度与实际学习活动分开展示：生成日的卡片读取截至当前的已答/必做数量，实际活动按首次有效提交的中国时间计数；整组完成仍按实际完成时间推进阶段。同账号、同复习任务/阶段/到期时间、同题目快照的重印共享判题记录，统计去重；写入先锁定账号并在原事务内读取共享记录，不更改旧 PDF、历史日期或用量。无法还原的旧清单明确显示待核对，不推测完成。

学习进度日历中的待复习选题与错题本共享同一份当日选题状态和 PDF 生成接口；用户可在日期详情中勾选后直接生成。当日计划已生成时，两个页面都锁定选题并复用原 PDF，不建立平行生成流程。

本地模拟复用同一注册状态机、MySQL 适配器、迁移和 ASGI 边界，仅替换短信与 CAPTCHA 外部适配器。localhost 启动器会在验证码申请成功的响应中附加本地测试码，页面明确标记并自动填入；生产装配不会返回该字段。使用 `python -X utf8 -B scripts/local_env.py serve --enable-harness-model --enable-harness-ui` 时，启用固定版 DeepSeek Harness 数学运行时及其原生 Web 界面；不带模型参数时既有产品模型接口稳定返回 `model_unavailable`。官方 Web Host 使用隔离的 `data/runtime/deepseek-harness-web-home` 和启动期随机回环端口，由本地启动器从私有 stdout 管道读取；stdout 与 stderr 经过回环权威地址掩码后才写日志。Windows 以明确非零的挂起标志创建子进程、绑定 `KILL_ON_JOB_CLOSE` Job，并用 `DWORD` 返回值验证恢复前挂起计数；Unix 使用独立进程组，保证主服务退出时回收整棵进程树。后台启动在创建子进程前独占领取 PID 记录，重复、并发或遗留记录均失败闭合；正式记录同时绑定 PID、创建时间和可执行文件，停止时身份不一致即拒绝杀进程，防止 PID 复用或所有权覆盖。若正式身份写入与新进程树回收同时失败，声明会转为同样可验证的 `recovery` 身份，后续停止仍有精确恢复依据。浏览器只访问 `http://127.0.0.1:8000`：公开面仅包括认证/协议外壳、品牌资源与只读 manifest；根页面、产品脚本、Harness 静态资源、插件资源、`/api/*` 和 WebSocket 均先经过服务端会话、Host 与同源 Origin 校验。manifest 仅接受 `GET` 并由 Python 静态提供。网关只连接固定回环上游，请求和响应头均使用正向白名单，产品 Cookie、授权头、身份头与上游 Cookie 不跨边界，允许返回的定位、链接和 CSP 头会把内部权威地址改写为公共同源地址。回环连接在首个响应前被重置时，仅 `GET`、`HEAD`、`OPTIONS` 安全方法重试一次，业务写请求绝不自动重放；WebSocket 上游升级后异常断开返回 `1013`，所有桥接任务在退出前取消并收集。Uvicorn 运行环境必须安装 `websockets` 传输实现，PID 身份验证依赖 `psutil`。同步 MySQL、文件、PDF 和模型调用通过标准库线程执行，避免阻塞 ASGI 事件循环；Harness HTTP 代理当前仍以 300 MiB 上限缓冲请求体，上传原图的生产路径必须改为流式 OSS。MySQL 固定绑定 `127.0.0.1:3307`；模拟服务固定绑定 localhost，不能作为生产启动入口。
