...
| 文件上传、解析、审核、判题 | API + Codex CLI 会话适配器 | PNG/JPEG 可自动生成识别候选→自然语言追问/修正→确认判题→继续追问/修正→确认入本；模型不可用时安全停止且不显示大表单，PDF/DOCX 自动解析与生产异步 Worker 仍延期 |
