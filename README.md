...
个人数学错题本的独立 Web 项目。v0.4.0 目标基线将验证码登录与新用户注册拆分：已有账号使用手机验证码登录，新用户使用手机号、验证码和密码注册并确认协议，注册成功自动登录。用户仍不填写资料、不选择身份、不创建家庭，也不经过监护或实名流程。当前认证代码仍是可运行的 v0.3.3 单入口版本，v0.4.0 认证重构尚未实现。
