...
当前 `openapi/web-v1.json` 与代码仍为 v0.3.3 单入口 `/v1/auth/otp/request`、`/v1/auth/otp/verify`，会自动创建或复用账号，尚无密码、协议或场景隔离。它们是迁移起点而非 v0.4.0 目标契约。
...
当前机器可校验契约见 `openapi/web-v1.json`；v0.4.0 目标认证契约见 `docs/13-LOGIN-REGISTER-PRD.md`，完成实现和迁移后再升级 OpenAPI。错误统一为 `{error:{code,message,retryable,request_id}}`；页面只依赖稳定 `code`，不解析文案。
