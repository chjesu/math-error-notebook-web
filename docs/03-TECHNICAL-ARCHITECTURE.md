...
Web 版面向多用户、应用与数据库分离部署，使用 MySQL 作为业务与安全状态的唯一持久化数据库。首版采用“模块化单体 API + 异步任务进程 + MySQL”，复用现有错题本的题库、审核、推荐、复习和 PDF 质量门，不建立第二套题库，也不引入 Redis、微服务注册中心或消息队列。
...
安全边界：浏览器、短信供应商和模型均是不可信边界；模型不能直接写 MySQL。所有模型输出必须经过现有 `grade-preview → grade-commit`、`prepare-review-batch → verify-review-batch`、`assign-recommendations` 等质量门，Web API 仅封装这些权威入口。
...
| IP 前缀 | 5 秒 | 30 | 100 | IPv4 /24、IPv6 /56 聚合；同时保留精确 IP HMAC 审计 |
| 设备标识 | 10 秒 | 10 | 30 | 随机持久设备 ID，仅作为一个信号，不作为可信身份 |
| 租户 | 无单请求冷却 | 300 | 1,000 | 防单租户配置或业务被滥用 |
| 全局成本 | 无 | 按预算 | 按预算 | 达阈值触发成本熔断 |
