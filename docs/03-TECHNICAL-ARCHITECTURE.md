...
> v0.3.2 产品边界：手机号验证后直接使用个人错题本，以 `user_id` 隔离。本文后续出现的家庭、租户产品权限或监护流程属于早期设计，等待 `architecture_and_contract` 步骤形成迁移方案；不得继续作为前台入口或注册阻断。
...
- `POST /v1/auth/otp/verify` 只区分“验证成功”和“验证码无效或已过期”，不返回“手机号未注册”。成功后统一创建或恢复会话，并直接进入个人错题本；不得进入资料完善、身份、家庭或监护流程。
