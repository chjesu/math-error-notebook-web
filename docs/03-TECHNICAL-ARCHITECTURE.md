...
当前 `openapi/web-v1.json` 与代码已升级为 v0.4.0 四接口：登录/注册验证码申请与验证分场景，注册原子保存密码、协议并创建会话；旧 v0.3.3 单入口仅作为迁移起点。真实 MySQL smoke、134 项测试、独立安全复核和浏览器验收已通过；生产门禁仍未完成。
...
当前机器可校验契约见 `openapi/web-v1.json`（30 paths）；认证细则见 `docs/13-LOGIN-REGISTER-PRD.md`。错误统一为 `{error:{code,message,retryable,request_id}}`；页面只依赖稳定 `code`，不解析文案。生产切换前仍需完成真实供应商、观测、压测、灾备和安全复核。
