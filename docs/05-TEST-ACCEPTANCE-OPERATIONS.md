...
首版验收以“功能正确、租户隔离、验证码成本可控、不可枚举账号、会话可撤销、模型不能越权写库”为硬门槛。任何安全门失败均不得上线；性能优化不能通过放宽限流、跳过审计或让模型直接写库实现。

测试环境必须使用短信供应商沙箱或测试号码，禁止压测真实短信通道产生费用。所有安全测试数据使用专用租户并在测试后按保留策略清理。

## 2. 测试矩阵

| 编号 | 场景 | 操作与覆盖维度 | 预期结果 / 验收证据 |
|---|---|---|---|
| REG-01 | 新手机号注册 | 请求并正确验证 OTP | 创建一个用户和一个已验证手机身份；创建会话；挑战为 `VERIFIED` |
| REG-02 | 已注册手机号再次验证 | 与 REG-01 使用相同外部流程 | 不新建用户；正常登录；请求和成功响应不暴露“已注册” |
| REG-03 | 验证码错误 | 连续输入错误码 | 统一返回 `invalid_or_expired_code`；第 5 次后 `LOCKED`；不能再验证 |
...
| RL-02 | 手机号小时/日限流 | 单号码跨设备、跨 IP 请求 | 达 5/小时或 5/日后停止发送，不能通过换设备/IP 绕过 |
| RL-03 | IP 限流 | 同 IP/前缀请求多个号码 | 达阈值升级 CAPTCHA，硬上限后停止发送 |
| RL-04 | 设备限流 | 同设备切换 IP 与号码 | 设备计数持续有效；达到硬上限后停止发送 |
| RL-05 | 租户限流 | 多 IP、多设备、多号码并发 | 达租户上限后该租户停止发送，不影响其他租户 |
...
| SMS-01 | 供应商受理/拒绝/超时 | 模拟三种结果 | 状态正确；超时不盲目跨供应商重发；响应仍不枚举账号 |
| SMS-02 | 提交响应解析 | `00`、`03`、拒绝、乱码、缺流水号 | 仅 `00`/`03` 且含流水号标记 SENT；其余失败关闭；错误不泄露敏感字段 |
| COST-01 | 预算 80% | 推进租户与全局预算 | 产生告警并强制 CAPTCHA；审计包含范围和窗口 |
| COST-02 | 预算 100% | 继续请求 | 新短信熔断；已发送挑战仍可验证；其他未熔断范围按策略运行 |
| BL-01 | 黑名单 | 手机、IP 前缀、设备、租户分别命中 | 不调用供应商；外部统一 202；内部记录明确原因码 |
| ENUM-01 | 账号枚举 | 对存在/不存在手机号各运行 1,000 次 | HTTP 状态、结构、文案一致；响应时间分布无显著可利用差异 |
| ENUM-02 | 错误旁路 | 比较限流、拉黑、供应商失败和正常受理 | 外部响应不泄露策略、供应商或账号状态；仅安全日志可区分 |
...
| TEN-01 | 租户隔离 | 用 A 租户身份访问 B 租户资源 | API 返回通用无权/不存在响应；数据库查询必须带租户边界；审计告警 |
| DB-01 | 敏感数据 | 检查 DB、应用日志、追踪与异常 | 无明文 OTP、会话 token、CAPTCHA token；手机号/IP 仅加密或 HMAC 形式 |
| DB-02 | 事务故障 | 在计数、挑战、用户创建阶段注入失败 | 回滚保持一致；不产生无计数挑战、重复用户或半登录状态 |
...
1. 同一 `(tenant_id, phone_hmac)` 同时最多一个未过期、可验证挑战；并发签发后旧挑战必须取消。
...
6. 所有用户业务表查询和唯一约束均包含明确的租户语义；跨租户请求不能只依赖前端过滤。
...
- 分布式低速攻击：多 IP、多设备围绕同一租户和成本预算持续请求。
...
使用同一网络条件，准备等量的已注册和未注册号码，对 OTP 请求及验证接口分别进行至少 1,000 次采样。比较 HTTP 状态、响应 JSON 字段、正文长度、错误码和延迟分布。验收要求：协议内容完全一致；延迟 P50/P95 差异不超过基线抖动预算，统计检验发现稳定差异时必须修复，不能仅靠增加固定 sleep 掩盖数据库分支。
...
- KMS 中已配置手机号加密密钥与 OTP HMAC pepper，应用配置和镜像不含密钥；
...
- 审计事件可追踪到 `request_id`，但不包含明文手机号、验证码或 token；
- 租户隔离测试通过，模型调用保持只读，所有题库写入仍经过项目质量门。

## 7. 监控与告警

| 指标 | 告警建议 | 处理目标 |
|---|---|---|
| OTP 请求量 / 实际发送量（按租户、IP、设备、手机号） | 5 分钟高于近 7 日同时间 P99 或发送比异常 | 5 分钟确认 |
...
1. 确认全局、租户、供应商预算和实际受理数，开启或保持成本熔断。
2. 按 HMAC 主体查看 IP 前缀、设备、手机号和租户聚类，不导出明文号码。
3. 提升 CAPTCHA 或添加有时限的黑名单；不得删除审计或放宽其他租户限制。
...
2. 修复差异响应；怀疑 token 泄露时按用户、租户或全局撤销会话并轮换密钥。
...
- 每月复核黑名单永久项、租户预算、CAPTCHA 误拦率和账号枚举回归；每次短信供应商 SDK 或反向代理配置变更都重跑相关矩阵。
...
每次候选发布保留以下证据：测试版本与提交号、迁移校验结果、测试矩阵逐项结果、压测参数及供应商实际受理数、账号枚举响应对比、MySQL 锁等待分位数、预算熔断演练、备份恢复记录。证据不得包含明文手机号、验证码、会话或 CAPTCHA token。

## 12. 产品学习闭环验收

除注册安全矩阵外，公开 MVP 还必须覆盖 PRD v0.3.1 的 GATE-B。以下是最低验收矩阵：

| 需求 | 关键场景 | 必要证据 |
|---|---|---|
| `STUDENT-001` | 家庭/学生切换、跨租户和跨学生读写；学生固定为未成年人 | 权限负向测试、缓存与任务作用域测试、拒绝 not_required 状态 |
...
| `PRIV-002` | 导出越权、链接过期和监护代办范围 | 导出内容对账、下载权限与过期证明 |
| `PRIV-003` | 注销残留、家庭所有权和共享数据影响 | 数据分类对账、会话与对象链接失效证明 |
...
1. 成年监护人注册并创建家庭与学生，学生档案初始为 pending；
2. 完成独立监护同意后启用学生学习功能，并验证学生不能自行批准；
3. 上传含题目和错误作答的样本；
4. 修正一次识别候选并冻结新版本；
5. 生成判题候选，确认后写入正式错题；
...
- 分析事件不含手机号、验证码、题目正文、学生姓名、原图地址、下载 token 或模型完整正文。
