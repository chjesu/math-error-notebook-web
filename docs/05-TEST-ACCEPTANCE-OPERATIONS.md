...
首版验收以“注册后直接使用、user_id 隔离、验证码成本可控、不可枚举账号、会话可撤销、模型不能越权写库”为硬门槛。任何安全门失败均不得上线。
...
- 每月复核黑名单永久项、账号预算、CAPTCHA 误拦率和账号枚举回归；每次短信供应商 SDK 或反向代理配置变更都重跑相关矩阵。
...
除注册安全矩阵外，公开 MVP 还必须覆盖 PRD v0.3.2 的直接使用和学习闭环。以下是最低验收矩阵：

| 需求 | 关键场景 | 必要证据 |
|---|---|---|
| `ACCOUNT-001` | 新手机号验证后直接进入工作台；无任何资料、身份、家庭或监护步骤 | 真实 HTTP E2E、首屏与上传入口验收 |
