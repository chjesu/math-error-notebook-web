...
- 本地 Harness 会在浏览器中把当前不透明会话编号绑定到 8000 端口已经认证的个人账号。最新用户消息含图片时，模型立即且恰好调用一次 `process_error_notebook_attachments`，不先识别或判题；Host 读取本轮全部附件字节，通过仅限回环地址和启动期随机令牌保护的接口创建一个持久批次。Python 双工作线程在 MySQL 两个固定槽位和 fencing token 保护下依次拆题、独立解题、判题、交叉验证并入本。工具轮询权威批次回执，浏览器扩展同时通过 SSE 显示进度；刷新后按 `Last-Event-ID` 重放。`confirm_error_notebook_entry` 只保留给其他已经生成冻结候选的流程；令牌、内部批次状态和模型提示不写入浏览器或日志。
