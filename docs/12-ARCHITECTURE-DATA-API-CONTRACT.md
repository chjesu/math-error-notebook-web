...
当前机器契约：`openapi/web-v1.json`。当前 36 条路径覆盖登录/注册四接口、会话、工作台、文件与任务、官方 Codex app-server 识别/判题候选与事件流、人工回退、正式入本、错题、已验证推荐、今日复习、进度、练习 PDF、敏感导出和注销。所有领域写接口均声明 `400`（请求/状态不合法）与 `403`（无操作权限）响应，不能只在认证接口声明错误。
...
- v0.4.0 已通过 0005—0010 前向迁移建立密码凭据、协议确认、挑战场景、注销恢复字段、文件上传幂等键映射和 Codex thread 持久映射；禁止改写0001—0004。
