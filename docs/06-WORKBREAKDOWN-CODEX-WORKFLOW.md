...
本文件定义 Web 版建设过程中任务如何拆分、领取、交付、复核和恢复，以及 Codex CLI 如何按难度选择模型。固定岗位、子智能体能力、并行波次和执行命令见 `14-CODEX-MULTI-AGENT-TEAM.md`。
...
| 个人数据隔离和服务边界 | C | I | A/R | C | C | C | I | I | C | C | I |
...
禁止以下交付方式：只报告“测试通过”但无命令/报告；以模型自评代替复核；将未确认候选直接写库；为了通过测试降低现有质量门；把跨用户测试、失败恢复或安全负向测试留到上线后。
...
`project` 模板覆盖产品基线到云部署批准；`team` 模板覆盖团队章程、岗位设计、编排、安全治理和独立验证。`model_task` 只表示应由哪个 Codex 路由做离线只读辅助，不授权自动外发；调用仍须显式数据发送授权。
...
| L1 需求复核 | 普通需求完整性、文案和验收标准 | Luna low | 认证、隐私、迁移或跨用户隔离风险直接升 Sol |
...
4. 看板按用户伪标识、任务、模型、版本和成功率汇总，不能显示手机号或其他个人资料。
