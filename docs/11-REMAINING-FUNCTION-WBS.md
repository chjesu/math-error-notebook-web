...
本文把李兆霖数学错题本 Web 版的剩余功能拆成可领取、可验证、可恢复的工程任务。它只规划当前 MVP：手机号验证后直接使用个人错题本，不建设资料、身份、家庭、学生档案、监护或实名流程。
...
| 能力 | 当前证据 | 判断 |
|---|---|---|
| 产品范围 | PRD、功能设计、测试验收文档 v0.3.2 | 已确认 |
| UE/UI/交互 | 核心流程、响应式规范、正式品牌资产 | 已确认 |
| 手机验证码基础 | 注册状态机、MySQL Store、ASGI、短信和 CAPTCHA 适配器 | 已实现旧口径，待简化 |
...
| 在建内容 | 当前文件 | 必须处理的问题 |
|---|---|---|
| 注册状态机 | `services/web_auth/registration.py` 等 | 仍要求 display_name、birth_date 和监护同意，必须改成验证码后直接建会话 |
...
上述未提交文件属于有效在建工作，后续任务应基于其可复用部分修改，不应删除后重写整套实现。
...
```mermaid
flowchart LR
    BASE[已完成：产品、UX、品牌、认证基础] --> C[里程碑 C：v0.3.2 契约收敛]
    C --> A[并行线 A：账号与会话]
...
## 5. 里程碑 C：v0.3.2 架构与契约收敛

此里程碑完成前，不继续扩展旧 tenant/student/guardian 模型，也不应用当前 0002 领域迁移。

| ID | 任务 | 角色 | 依赖 | 交付物 | 完成证据 |
|---|---|---|---|---|---|
| ARCH-001 | 冻结账号与授权边界 | ARCH + SEC | 产品与 UX 基线 | 当前会话 user_id、账号状态、会话撤销和跨用户拒绝规则 | 权限矩阵；无客户端 user_id 作用域参数；安全复核记录 |
...
| ARCH-004 | 冻结 OpenAPI 与稳定错误码 | ARCH + BE + FE | ARCH-001、ARCH-003 | 账号、上传、任务、确认、判题、错题和工作台最小 API | OpenAPI 校验通过；请求/响应样例；错误码与页面状态逐项映射 |
| ARCH-005 | 冻结迁移顺序与回滚 | DATA + ARCH | ARCH-002 | 已应用 0001 的前向修订、未应用 0002 的重构策略、迁移账本语义 | 空库、现有本地库、重复执行和失败恢复演练计划 |
| ARCH-006 | 对齐工作流与文档 | DM + ARCH | ARCH-001—005 | `WEB-PRD-003` 作为新基线；旧工作流状态仅作历史证据 | 工作流 artifact、依赖和任务 ID 与本 WBS 一致 |
...
|---|---|---|---|---|---|
| AUTH-101 | 移除资料和监护注册门 | BE | ARCH-001、ARCH-004 | `services/web_auth/registration.py` | 新号/旧号验证码成功均直接产生会话；无姓名、出生日期、监护参数；状态机测试 |
| AUTH-102 | 前向修订账号持久化 | BE + DATA | ARCH-005、AUTH-101 | auth Store 与新迁移 | 已应用 0001 不被重写；不再依赖资料字段；事务和并发测试 |
| AUTH-103 | 对齐 HTTP 注册契约 | BE | AUTH-101、ARCH-004 | `services/web_auth/asgi.py`、bootstrap | 验证成功直接返回工作台可用状态；Cookie 安全属性保持；等价公开响应 |
| AUTH-104 | 会话查询与撤销 | BE + SEC | AUTH-102 | auth API/Store | 当前会话、退出当前设备、全端失效；旧 Cookie 访问失败 |
| AUTH-105 | 注册安全回归 | QA + SEC | AUTH-101—104 | tests 与演练证据 | 重放、枚举、并发消费、限流、供应商未知结果和敏感日志扫描通过 |
...
| FE-102 | 实现手机号验证页 | FE | FE-101、ARCH-004 | auth page | 同一登录/注册入口；按需 CAPTCHA；无资料和身份步骤；键盘可用 |
...
| ENV-102 | 扩展本地环境验收 | SRE + QA | AUTH-103、DATA-105、FILE-104 | local_env smoke | 注册、建会话、上传和 user_id 查询使用真实 MySQL/HTTP 跑通 |

基础完成门：四条并行线的目标任务通过复核，且本地环境能以新 Schema 启动；旧 tenant/student/guardian 不再是产品或授权前置条件。

## 7. 里程碑 V：首个可验证纵向闭环

目标流程：`手机号验证 → 空工作台 → 上传一道题 → 确认题干与作答 → 得到判题候选 → 确认入本 → 错题详情/列表`。
...
| SEC-501 | 系统威胁与隐私回归 | 独立 SEC | OPS-501—503、PERF-501 | 攻击面、越权、滥用、恶意文件、密钥和依赖审查 | 高危清零；实现者不是唯一审核人 |
...
| 活跃需求 | 主任务 |
|---|---|
| ACCOUNT-001 | ARCH-001、AUTH-101—104、WORKBENCH-201 |
| AUTH-001、AUTH-002 | AUTH-101—105、PROD-601—602 |
...
| OPS-001 | REVIEW-301、OPS-501—503、PERF-501 |

## 13. 并行与文件所有权

契约收敛后可并行：

- AUTH-101—105：只改认证状态机、Store、API 和对应测试；
...
2. 并行推进 AUTH-101、DATA-101、FILE-101、FE-101 和 ENV-101。
...
3. 新用户验证码成功后直接使用，不出现资料、身份、家庭、学生、监护或实名流程；
