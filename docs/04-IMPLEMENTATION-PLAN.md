...
v0.4.0 目标基线（配置化并须持续压测校准）：同手机号冷却60秒、每小时最多5次、滚动24小时最多10次；同精确IP每分钟10次、每小时20次；同IP前缀每小时30次；同设备每小时10次、滚动24小时20次；全局每日10,000次。login/register 场景分别审计但共享手机号和平台预算。现有 v0.3.3 代码仍是手机号5次/日，必须在重构中迁移并补边界测试。

完成证据：登录与注册页面/API端到端测试；注册账号/密码/协议/会话原子事务测试；验证码跨场景与并发原子消费测试；按手机号/IP/设备/网段的轰炸模拟；重放、批量枚举、时钟漂移和供应商超时测试；密码敏感数据扫描；限流指标和告警可见。
