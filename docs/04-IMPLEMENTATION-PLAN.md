...
> 文档版本：`v0.4.0`
> 基线日期：`2026-08-23`
> 状态：v0.4.0 本地候选已落地；本地手工闭环已验收，生产批次与门禁仍待执行
...
产品范围以 `02-PRODUCT-REQUIREMENTS.md` v0.4.0 和 `13-LOGIN-REGISTER-PRD.md` 为准。所有普通用户采用同一数据模型，但认证拆为验证码登录与新用户注册两个场景；注册成功自动登录，不经过资料、身份、家庭、学生档案、监护或实名流程。个人业务数据以服务端会话的 `user_id` 隔离。

首期不建设独立向量数据库。题目召回先使用 MySQL 结构化标签、全文索引和项目现有语义重排；只有离线评测证明召回质量或延迟不能达标时，才通过 ADR 引入向量检索。

## 2. 实施原则

1. **先边界、后功能**：先确定 user_id 隔离、会话认证、写入质量门和审计边界，再开放业务入口。
...
6. **隐私最小化**：手机号、验证码、密码和作业图片按最小权限处理；不采集姓名、昵称、年级、年龄、身份、身份证或人脸资料。
...
- 冻结 user_id 授权边界、数据所有权、保留期和删除策略；
...
产品映射：`ACCOUNT-001`、`AUTH-001`、`AUTH-002` 和 `PRIV-001`。

交付：

- 用户、会话、个人错题本作用域和审计表；
- 验证成功后原子创建账号与会话并直接返回可用工作台；
- 删除资料完善、身份选择、家庭、学生档案、监护同意和实名阻断；
- `发送验证码 → 校验验证码 → 创建账号/会话` 完整状态机；
- login/register 挑战用途隔离，登录不得建号、注册不得覆盖已有账号；
- 注册密码凭据安全存储和协议版本确认记录；
- 手机号、IP、设备指纹、账号、场景五层限流；
- 图形验证码/行为挑战、冷却时间、日配额、供应商熔断和异常告警；
- 验证码哈希存储、短时有效、尝试次数限制、原子消费、防重放；
- 按产品要求返回 `phone_not_registered`/`phone_already_registered` 跳转反馈，并用限流、CAPTCHA、告警和独立安全复核控制枚举风险；
- 风控命中时不向模型发送任何验证码、手机号或设备标识。

v0.4.0 本地候选基线：同手机号冷却60秒、每小时最多5次、滚动24小时最多10次；login/register 场景分别审计但共享手机号和平台预算；精确 IP、IP 前缀、设备和平台维度按实现配置执行。真实短信/CAPTCHA、供应商熔断、告警、压测校准和生产安全复核仍待完成；旧 v0.3.3 限流口径仅保留为历史迁移背景。

本地完成证据：登录与注册页面/API 端到端测试；注册账号/密码/协议/会话原子事务测试；验证码跨场景与并发原子消费测试；重放、批量枚举和密码敏感数据扫描；166 项测试、真实 MySQL smoke、官方 Codex app-server 连续会话与数学候选实测和浏览器验收通过。生产完成还需真实供应商小流量、时钟漂移/超时演练、限流指标告警和其他生产门禁。

### 批次 2：领域数据与单机库迁移

产品映射：`ACCOUNT-001`、`CONTENT-001`，并为 `ERROR-001`、`REVIEW-001`、`PRIV-002` 提供权威数据基础。

交付：

- MySQL 中的题库、题目版本、来源、验证记录、错题、作答、推荐、复习任务和文件元数据；
- 所有个人业务表具有明确的 `user_id` 归属并由服务层强制过滤；
- 对早期 `tenant_id/student_id`、家庭、成员和监护字段形成可回滚迁移方案，不能继续作为产品前置条件；
- SQLite 到 MySQL 的只读抽取、校验、导入、对账和回滚工具；
- 内容哈希去重、外键、唯一约束和审计事件；
- 现有 `notebook.py` 领域规则封装为服务层能力，避免出现平行验证器或推荐器。

完成证据：匿名化快照迁移两次结果一致；总数、哈希、关联完整性和抽样内容对账通过；跨用户访问测试全部拒绝；回滚不丢原始数据。
...
- 合规：隐私最小化、数据保留/删除和第三方授权检查完成；不存在实名、身份或监护资料采集。

## 8. 项目完成定义

“代码已合并”不等于项目完成。只有批次 0—5 的完成证据均已归档，新账号验证后可直接进入错题本，生产灰度指标达到既定 SLO，至少完成一次从注册到错题复习 PDF 的真实端到端流程，并完成备份恢复、验证码攻击和异步任务中断三类演练，项目才可标记为上线完成。
