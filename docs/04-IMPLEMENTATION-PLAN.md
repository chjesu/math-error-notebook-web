...
产品范围以 `02-PRODUCT-REQUIREMENTS.md` v0.3.1 为准。所有学生均为未成年人；成年人账号只承担监护、家庭、教师等角色。公开 MVP 有两道不可替代的门：GATE-A 账号与租户基础、GATE-B 错题学习闭环。批次可以独立交付工程证据，但注册模块完成不能替代完整学习闭环。
...
- 开通时区分成年人角色与学生角色；学生不能创建家庭、创建学生档案或自行批准监护同意；
- 所有学生档案初始监护状态为 `pending`，不存在 `not_required` 产品状态；
...
- 数据库和服务层必须拒绝学生档案进入“无需监护同意”，仅允许 `pending/active/revoked`；
...
- 合规：隐私最小化、数据保留/删除、所有学生的监护同意和第三方授权检查完成。
