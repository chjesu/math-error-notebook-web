...
本地完成证据：登录与注册页面/API 端到端测试；注册账号/密码/协议/会话原子事务测试；验证码跨场景与并发原子消费测试；重放、批量枚举和密码敏感数据扫描；165 项测试、真实 MySQL smoke、官方 Codex app-server 连续会话与数学候选实测和浏览器验收通过。生产完成还需真实供应商小流量、时钟漂移/超时演练、限流指标告警和其他生产门禁。
