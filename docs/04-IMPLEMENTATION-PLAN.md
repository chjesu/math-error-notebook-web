...
产品范围以 `02-PRODUCT-REQUIREMENTS.md` v0.3.2 为准。所有普通用户采用同一模型：手机号验证成功后直接进入个人错题本，不经过资料、身份、家庭、学生档案、监护或实名流程。个人业务数据以服务端会话的 `user_id` 隔离。
...
6. **隐私最小化**：手机号、验证码和作业图片按最小权限处理；不采集姓名、昵称、年级、年龄、身份、身份证或人脸资料。
...
当前已实现基线（配置化并须持续压测校准）：同手机号冷却 60 秒、1 小时和滚动 24 小时均最多 5 次；同精确 IP 每分钟 10 次、每小时 20 次；同 IP 前缀每小时 30 次；同设备每小时 10 次、滚动 24 小时 20 次；全局每日 10,000 次。现有内部 tenant 限额在迁移期间可保留为兼容字段，但不能要求用户创建家庭或选择身份。
