...
“清空会话”固定在会话区右上角，使用次要按钮样式。点击后必须说明“未完成题目和上传附件将从工作台移除，已入本错题不受影响”；确认成功后回到空白欢迎态。

底部会话框采用“文字区 + 工具栏”的两层结构：文字输入始终打开；工具栏左侧是添加附件，右侧是发送按钮。题卡不再显示“确认并判题”或“确认入本”；上传后系统按顺序自动判题和整理，界面只更新各题状态与进度。证据不足时保留文字输入和“跳过本题”，供用户补充、修正或跳过。不使用伪装成按钮的隐藏单选框、下拉菜单，也不暴露模型、权限或运行环境。Enter 发送、Shift+Enter 换行，中文输入法选字时 Enter 不得误发。
...
PNG/JPEG 缩略图本身是“查看原图”入口，点击后使用浏览器原生模态预览并支持 Esc/关闭按钮返回。单张图片含多道题时，识别必须遍历整图，按阅读顺序逐题拆分，并把邻近的手写步骤、勾选、圈选或最终答案关联到对应题目。全部识别结果一次性展示在同一条助手消息内，一道题一个富文本题卡；界面明确显示总题数和每题状态，不能把“一张文件”误报为“一道题”。服务端仍按阅读顺序逐题冻结版本、判题和入本，避免跨题串写。同一未冻结图片再次上传时允许刷新旧识别候选，使模型协议升级或识别修正能够对已去重文件生效；已冻结或已进入判题的题目不得被刷新覆盖。
...
题干与作答候选集中在一条只读助手消息中，所有修正只从底部聊天框输入：
...
- 页面不出现独立题干/作答文本框；候选正文始终只读，当前题卡只保留确定性的确认和跳过按钮。
- 自然语言修正经智能助手返回完整结构化候选并创建新输入版本；旧分析立即失效。
- 上传批次后，系统按阅读顺序自动进入版本冻结门；普通对话只用于补充、修正和追问，不触发重复写库。
- 多题试卷一次展示全部题卡并标记当前题；当前题判定正确时自动激活下一题，错误或部分正确时自动入本后继续，证据不足时留在当前题等待补充。
