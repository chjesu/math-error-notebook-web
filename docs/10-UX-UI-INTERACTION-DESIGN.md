...
底部会话框采用“文字区 + 工具栏”的两层结构：文字输入始终打开；工具栏左侧是添加附件和平铺的单选操作项，右侧是唯一发送按钮。所有当前可用动作同时可见，不使用下拉菜单，也不暴露模型、权限或运行环境：无活动题目时为“询问”，题干确认阶段为“询问或修正 / 确认并判题 / 下一题”，判题阶段为“询问或修正 / 确认入本（仅错误或部分正确时出现）/ 下一题”。选择确认类动作后可以直接发送，无需输入暗号；若用户开始输入文字，单选项自动回到“询问或修正”，避免误把草稿当成确认。Enter 发送、Shift+Enter 换行，中文输入法选字时 Enter 不得误发。
