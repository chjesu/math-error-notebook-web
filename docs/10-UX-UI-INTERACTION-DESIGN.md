...
助手在同一条消息中按真实执行状态流式更新“准备发送附件 → 正在上传附件（文件序号、名称与可得百分比）→ 正在创建录入任务 → 文件已准备好”。当前本地版本未接入自动识别时，必须明确说明正在等待用户核对题干与作答，不得显示虚假的 OCR、AI 分析或预计剩余时间。题干确认、判题候选和入本确认继续作为助手消息依次出现在该对话流中；不跳出大卡片、不弹出多余对话框。
