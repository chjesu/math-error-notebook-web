...
底部会话框采用“文字区 + 工具栏”的两层结构：文字输入始终打开；工具栏左侧是添加附件，右侧是发送按钮。题干确认动作放在当前题卡上，以真正的按钮平铺显示“确认并判题 / 跳过本题”；判题完成后，输入框工具栏只显示当前可用的“确认入本（仅错误或部分正确时出现）/ 下一题”。不使用伪装成按钮的隐藏单选框、下拉菜单，也不暴露模型、权限或运行环境。输入文字始终表示询问或修正；确认类按钮点击后立即执行，不要求再点发送。Enter 发送、Shift+Enter 换行，中文输入法选字时 Enter 不得误发。
...
PNG/JPEG 缩略图本身是“查看原图”入口，点击后使用浏览器原生模态预览并支持 Esc/关闭按钮返回；预览不承担确认业务。单张图片含多道题时，识别必须遍历整图，按阅读顺序逐题拆分，并把邻近的手写步骤、勾选、圈选或最终答案关联到对应题目。全部识别结果一次性展示在同一条助手消息内，一道题一个富文本题卡；界面明确显示总题数和每题状态，不能把“一张文件”误报为“一道题”。服务端仍按阅读顺序逐题确认、判题和入本，避免跨题串写。同一未确认图片再次上传时允许刷新旧识别候选，使模型协议升级或识别修正能够对已去重文件生效；已确认或已进入判题的题目不得被刷新覆盖。
...
- 只有点击“确认并判题”或精确发送同名口令才进入版本确认门，普通对话不得触发判题。
- 多题试卷一次展示全部题卡并标记当前题；当前题判定正确时自动激活下一题，错误或部分正确时等待确认入本，证据不足时留在当前题补充。
