...
> 设计日期：2026-08-22
>
> 状态：核心流程可评审；API 与数据契约待补充
>
> 上游：02-PRODUCT-REQUIREMENTS.md、09-PRODUCT-FUNCTION-DESIGN.md

## 1. 设计结论

产品采用“任务优先、内容优先”的个人错题本界面。用户完成手机号验证后直接进入工作台；任何页面都不要求姓名、昵称、年级、身份、家庭、监护或实名信息。
...
“录入错题”是导航上方固定主按钮，不再作为与其他栏目同级的普通菜单。设置与隐私放在侧栏底部；处理中任务放在右上角任务入口。
...
整体为安静、可信的“纸张与批注”风格：暖灰页面背景、白色内容面、深色正文、蓝色主操作、少量橙色批注。数学题、作答图片和错误步骤是视觉主角，装饰元素保持克制。
...
| 主色 | primary | `#2457D6` | 唯一主操作、当前导航、焦点 |
| 批注 | annotation | `#B85C00` | 首错步骤和学习提示 |
...
- “登录 / 注册”使用同一按钮，不询问账号是否存在。
- CAPTCHA 只在服务端要求时插入手机号与验证码之间。
- 验证成功后直接替换为工作台，不增加欢迎页或资料步骤。
...
有数据时按“待确认 → 今日到期 → 处理中/失败 → 近期错题”排序，只展示真实存在的区块，不用空数据卡填满页面。
...
- 默认进入查看状态，点击具体字段后才编辑，避免整页输入框噪声。
- 修改题干或作答后显示“分析将基于新版本重新开始”。
- 多题试卷使用“第 2/8 题”与上一题/下一题，不使用横向缩略图长列表。
- 保存草稿自动完成，不再增加“保存”和“下一步”两个同级主按钮。

### 6.5 P-GRADE-RESULT 分析结果
...
### 6.6 P-ERROR-LIST 与 P-ERROR-DETAIL
...
### 6.8 P-PRACTICE、P-PROGRESS 与 P-SETTINGS
...
| 场景 | 触发 | 界面状态 | 用户可做什么 |
|---|---|---|---|
| 验证码已提交 | 点击登录/注册 | 按钮加载且禁止重复提交 | 等待；失败后按原字段修正 |
