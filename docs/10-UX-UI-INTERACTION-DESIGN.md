...
“录入错题”是工作台的唯一主任务，采用类似通用 AI 助手的单聊天流：中间只显示欢迎语和本次录入步骤，底部只显示一个附件输入框。错题、复习、练习和进度数据不作为统计卡片混入工作台。
...
助手在同一条消息中按真实执行状态更新“正在上传附件（文件序号、名称与可得百分比）→ 正在建立错题会话 → Codex 正在识别题目与作答 → 文件已准备好”。文件建立 intake 后，底部附件输入框立即成为文字聊天框；用户可以像 Codex 会话一样继续追问、修正题干/OCR、核对判题和要求重算。模型不可用时必须安全停止，不显示虚假的 OCR、AI 分析或预计剩余时间。题干候选、判题候选和入本结果作为只读助手消息依次留在对话流中；不得跳出大卡片、在文字流中插入表单或弹出多余对话框。
...
- 自然语言修正经 Codex 返回完整结构化候选并创建新输入版本；旧分析立即失效。
