...
本次协作按产品/UX、实现、安全与文档岗位复核，结论统一为：产品是个人错题本，不收集昵称、年级、身份、家庭、成员、学生档案、监护或实名信息；认证目标为登录验证码申请/验证、注册验证码申请、注册完成四接口；本地学习链路采用上传、Codex CLI 识别候选（或手工回退）、确认、Codex CLI 判题候选（或手工回退）、确认入本、已验证推荐、复习和 PDF。导出与注销必须敏感 OTP 二次验证并验证会话、任务和下载失效。

产品/UX 审查确认 v0.4.0 候选已具备独立认证状态、个人页面、任务恢复、异常分类和只读脱敏运营后台；后端/接口/浏览器 E2E、真实 MySQL smoke、230 项测试和官方 Codex app-server 连续会话与数学候选实测已完成。本地同步候选闭环不等于生产完成；生产异步 Worker、PDF/DOCX 自动解析、PWA、后台人工复核与敏感操作治理、生产恢复和云部署明确延期。
