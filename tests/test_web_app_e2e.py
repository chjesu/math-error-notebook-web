...
class NotebookE2ETests(unittest.TestCase):
    ...
    def test_phone_to_first_error_http_slice_and_cross_user_denial(self) -> None:
        ...
        self.assertEqual(len(self.call("/v1/errors?cause_code=algebra_transform", cookie=cookie)[2]["items"]), 1)
        self.assertEqual(self.call("/v1/errors?cause_code=calculation", cookie=cookie)[2]["items"], [])
        self.assertEqual(self.call("/v1/errors?cause_code=unknown", cookie=cookie)[0], 400)
        ...
        profile = self.call("/v1/progress/profile", cookie=cookie)
        self.assertEqual(profile[2]["cause_distribution"][0]["cause_code"], "algebra_transform")
        self.assertEqual(profile[2]["knowledge_radar"][0]["count"], 1)
        ...
        practice = self.call("/v1/practice-pdfs", method="POST", payload={"error_ids": [error_id], "mode": "self_test"}, cookie=cookie, idempotency_key="practice-0001")
        self.assertEqual(practice[0], 201)
        self.assertEqual(practice[2]["mode"], "self_test")
        self.assertEqual(self.call("/v1/practice-pdfs", method="POST", payload={"error_ids": [error_id], "mode": "review", "include_answers": False}, cookie=cookie, idempotency_key="practice-conflict")[0], 400)
        ...
        self.assertEqual(practice_history[2]["items"][0]["mode"], "self_test")
