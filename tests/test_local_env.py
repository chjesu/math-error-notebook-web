...
class LocalEnvironmentTests(unittest.TestCase):
    def test_startup_rejects_merge_markers_and_invalid_plugin_syntax(self) -> None:
        for source, returncode, message in (("<<<<<<< branch\n", 0, "merge markers"), ("bad js", 1, "invalid Harness JavaScript")):
            with self.subTest(message=message), mock.patch.object(Path, "read_text", return_value=source), mock.patch.object(
                local_env.shutil, "which", return_value="node"
            ), mock.patch.object(local_env.subprocess, "run", return_value=mock.Mock(returncode=returncode)):
                with self.assertRaisesRegex(RuntimeError, message):
                    local_env._check_harness_sources()

    def test_background_start_reports_ready_only_for_a_live_healthy_process(self) -> None:
        process = mock.Mock()
        process.poll.return_value = None
        with mock.patch.object(local_env.http.client, "HTTPConnection") as connection:
            connection.return_value.getresponse.return_value.status = 200
            local_env._wait_service_ready(process, "localhost", 8123)
