...
(async()=>{
  ...
  const recorded = id => ({type:'due',error_id:id,stage:2,question_text:'选择正确答案：A.1；B.2；C.3；D.4',first_error:'错因',knowledge_points:['知识点'],original_due_date:'2026-07-01'});
...
"""
    ...
    def test_workbench_is_the_official_deepseek_harness_surface(self) -> None:
        ...
        self.assertIn('return {schema: "math-notebook-transcription/v1", batch_ref: "", items: []}', host_plugin)
        ...
        self.assertIn('required: ["batch_ref", "items"]', host_plugin)
        ...
        self.assertIn("./notebook-protected-compaction.mjs", preset)
        ...
        for prompt in (patch, preset, runtime_config):
            ...
            self.assertNotIn('process_error_notebook_attachments 的 review 字段', prompt)
            self.assertIn('transcribe_error_notebook_attachments 的 review 字段', prompt)
            ...
            self.assertIn('每题 attachment_index/item_no', prompt)
            self.assertIn('batch_ref', prompt)
        ...
        self.assertIn("./notebook-protected-compaction.mjs", runtime_config)
        self.assertIn("./notebook-protected-pruner.mjs", runtime_config)
