...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_is_one_chat_flow_with_one_composer(self) -> None:
        ...
        self.assertIn('id="composer-actions" class="composer-actions" aria-label="当前题操作"', html)
        self.assertNotIn('name="composer-action"', html)
        ...
        self.assertIn("识别结果 · 共 ${intakeBatch.length} 道题", script)
        self.assertIn('className = `recognized-question-card is-${state}`', script)
        self.assertIn("setTimeout(processActiveIntake, 0)", script)
        self.assertIn("setTimeout(processActiveIntake, 3000)", script)
        self.assertIn('["model_network_error", "model_rate_limited"]', script)
        self.assertIn("3 秒后自动续跑", script)
        self.assertIn('resumeAutomaticIntake = stage === "intake"', script)
        self.assertIn('["waiting_confirmation", "confirmed"].includes(activeIntake.status)', script)
        self.assertIn("await confirmAndGrade()", script)
        self.assertIn("await commitCurrent()", script)
        self.assertIn('activeCandidate?.verdict === "unclear"', script)
        self.assertNotIn('button.dataset.intakeAction = value', script)
        self.assertIn('if (initialRender) requestAnimationFrame(() => activeCard?.scrollIntoView({block: "nearest"}))', script)
        self.assertNotIn("后面还有 ${pendingIntakes.length} 道题", script)
        ...
        self.assertIn("item.previewUrl || item.preview_url", script)
        self.assertIn("item.attachmentId || item.attachment_id", script)
        self.assertIn("result.items.map(item => item.attachment).filter(Boolean)", script)
        self.assertIn('item.status === "extracting" && item.attachment', script)
        self.assertIn('state: "recognition_failed"', script)
        self.assertIn("已恢复上次未完成的图片，点击重试即可继续识别。", script)
        self.assertIn("Array.isArray(item.attachments)", script)
        self.assertIn('id="clear-conversation"', html)
        self.assertIn('await api("/v1/conversations/latest", {method: "DELETE"})', script)
        self.assertIn("已经入本的错题不受影响", script)
        self.assertIn(".clear-conversation-button", css)
    ...
    def test_workbench_uses_harness_style_history_window_and_docked_interactions(self) -> None:
        ...
        self.assertIn('[["next", "跳过本题"]]', script)
        self.assertIn('const fixedMessages = {next: "下一题"}', script)
        self.assertIn('function selectedComposerAction()', script)
        self.assertIn('button.dataset.composerAction = value', script)
        self.assertNotIn('button.dataset.intakeAction = value', script)
        self.assertIn('async function sendMessage(preserveScroll = false)', script)
        self.assertIn('if (holdScroll) return', script)
        self.assertIn('chatInput.focus({preventScroll: holdScroll})', script)
        self.assertEqual(script.count('sendMessage(true)'), 1)
        self.assertIn('busy = false;\n      renderIntakeBatch();\n      setComposerState();', script)
        self.assertNotIn('radio.type = "radio"', script)
        ...
        self.assertIn("function wrapPlainMath(value)", script)
        self.assertIn('.replace(/√\\s*(\\([^()]*\\)|[A-Za-z0-9]+)/g, "\\\\sqrt{$1}")', script)
        self.assertIn('.replace(/\\^(-?\\d+)/g, "^{$1}")', script)
        self.assertIn('.replace(/π\\s*\\/\\s*(\\d+)/g, "\\\\frac{\\\\pi}{$1}")', script)
        self.assertIn("node.nodeValue = wrapPlainMath(node.nodeValue)", script)
        self.assertIn('output: "mathml"', script)
        self.assertIn('throwOnError: false', script)
        self.assertIn('replace(/^(\\s*[A-D][.、．]\\s*)\\\\+\\s*$/gm, "$1")', script)
        self.assertIn("renderMath(card)", script)
        self.assertIn('renderMath($("#error-detail"))', script)
        self.assertNotIn('new Set(["确认并判题", "确认题干与作答", "开始判题"])', script)
        self.assertNotIn('new Set(["确认入本", "确认写入错题本", "加入错题本"])', script)
        self.assertIn(".history-pagination", css)
        self.assertIn("#chat-input", css)
        self.assertIn(".composer-actions", css)
        self.assertIn(".composer-action.is-primary", css)
        self.assertIn(".recognized-question-card", css)
        self.assertNotIn(".question-card-actions", css)
        self.assertNotIn(".composer-action input:checked + span", css)
