...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_is_the_official_deepseek_harness_surface(self) -> None:
        ...
        self.assertIn('/v1/internal/harness/intake-batches', host_plugin)
        self.assertIn('x-lzlm-session-id', host_plugin)
        self.assertIn('while (!payload.terminal)', host_plugin)
        self.assertIn('Do not recognize or grade the images before calling', host_plugin)
        ...
        self.assertIn('/v1/intake/batches/active', plugin)
        self.assertIn('updated_after=${recoveryCursor}', plugin)
        self.assertIn('recovery_cursor', plugin)
        self.assertNotIn('const discoveryStartedAt = Date.now()', plugin)
        self.assertIn('new EventSource(`${productOrigin}${batch.events_url}`', plugin)
        self.assertIn('installIntakeBatchProgress(ctx)', plugin)
        self.assertIn('data-lzlm-batch-progress', plugin)
        self.assertIn('"progress", "item_completed", "batch_completed", "batch_failed"', plugin)
        self.assertIn('batch.item', plugin)
