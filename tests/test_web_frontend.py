...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_upload_supports_multiple_files_drag_and_paste(self) -> None:
        ...
        self.assertIn('window.addEventListener("dragenter"', script)
        ...
        self.assertIn('window.addEventListener("paste"', script)
        self.assertIn('window.addEventListener("keydown"', script)
        self.assertIn("navigator.clipboard.read()", script)
        self.assertIn("isInsideUploadSurface(event)", script)
        self.assertIn("function transferredFiles(transfer)", script)
        self.assertIn("transfer?.items", script)
        self.assertIn("mimeExtensions", script)
        self.assertIn("item.file.lastModified === file.lastModified", script)
        self.assertIn("已忽略 ${duplicates} 个重复文件", script)
        self.assertIn("for (const [index, file] of Array.from(files).entries())", script)
        self.assertIn("URL.createObjectURL(file)", script)
        self.assertIn("可点击选择，也可将多个文件拖到页面任意位置，或直接粘贴截图", html)
    ...
    def test_workbench_is_one_chat_flow_with_one_composer(self) -> None:
        ...
        self.assertIn('id="file-picker" class="attach-button" for="file"', html)
        self.assertIn("添加图片/文件", html)
        self.assertIn('id="file" name="file" type="file"', html)
        self.assertIn("multiple", html)
        self.assertNotIn('$("#file-picker").addEventListener', script)
        ...
        self.assertIn('const retryableUploadStates = new Set(["queued", "failed", "processing_failed", "recognition_failed"])', script)
        self.assertIn("retryableUploadStates.has(item.state)", script)
