...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_is_one_chat_flow_with_one_composer(self) -> None:
        ...
        self.assertIn('id="intake-context"', html)
        ...
        self.assertIn("appendAssistantProgress()", script)
        self.assertIn('className = "chat-progress-steps"', script)
        self.assertIn('"附件已安全保存"', script)
        self.assertIn('"已加入确认队列"', script)
        self.assertIn("item => !item.submitted", script)
