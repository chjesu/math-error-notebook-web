...
class PracticePdfMathTests(unittest.TestCase):
    ...
    def test_each_compact_question_heading_stays_with_its_answer_area(self) -> None:
        items = [
        ...
        self.assertTrue(any("原错题" in page and "原题作答区" in page for page in pages))
        self.assertTrue(any("同类型推荐题 1" in page and "推荐题作答区" in page for page in pages))
        text = "\n".join(pages)
        self.assertNotIn("Raaaaaaaaaaaa-01-AAAAAA", text)
        self.assertNotIn("Raaaaaaaaaaaa-02-BBBBBB", text)
