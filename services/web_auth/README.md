...
> 产品差异：本目录当前实现v0.3.3单入口验证码自动注册/登录；v0.4.0目标要求独立验证码登录和手机号注册，注册增加密码与协议确认。当前实现不再包含资料或监护门，但也尚未实现v0.4.0，详细差距见`docs/13-LOGIN-REGISTER-PRD.md`。
...
当前生产工厂仍是v0.3.3单入口契约。v0.4.0修改必须集中在权威注册状态机中，增加`purpose=login|register`隔离、密码凭据、协议记录、目标API、负向测试和前向迁移；在这些证据完成前不得把认证模块标为v0.4.0产品验收通过。
